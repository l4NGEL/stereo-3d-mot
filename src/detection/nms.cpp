...
std::vector<int> nms(const std::vector<cv::Rect2f>& boxes, const std::vector<float>& scores,
    ...
    for (int i = 0; i < static_cast<int>(boxes.size()); ++i) {
        if (scores[static_cast<std::size_t>(i)] >= score_threshold) order.push_back(i);
    }
    ...
    for (std::size_t i = 0; i < order.size(); ++i) {
        ...
        for (std::size_t j = i + 1; j < order.size(); ++j) {
            if (!suppressed[j] &&
                iou(boxes[static_cast<std::size_t>(order[i])],
...
std::vector<int> nmsClassAware(const std::vector<cv::Rect2f>& boxes,
                               ...
                               float score_threshold, int top_k) {
    ...
    for (std::size_t i = 0; i < boxes.size(); ++i) {
        ...
        shifted[i] = cv::Rect2f(boxes[i].x + off, boxes[i].y + off, boxes[i].width, boxes[i].height);
    }
